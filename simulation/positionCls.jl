...
function main()
	...
	for line in eachline(file)
		...
		if m.captures[2] == "LYS"
		    if occursin(r"\wZ\d?", m.captures[1])
			zpos = vcat(zpos, parse.(Float64, split(m.captures[4])))
		    end
		end
		...
		if m.captures[2] == "Cl-"
		    clcount += 1
		    NZpos = zpos[12*(clcount-1) .+ (1:3)]
		    HZav = [0.0, 0.0, 0.0]
		    for dim in 1:3
		        for hz in 1:3
		            HZav[dim] += zpos[12*(clcount-1) + 3*hz + dim]
			end
			HZav[dim] = HZav[dim]/3.0
		    end
